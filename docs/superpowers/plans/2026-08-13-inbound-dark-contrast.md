...
**Architecture:** 只调整 `templates/inbound.html` 的局部样式和 GYJ 状态卡标记，不触碰 CRM/GYJ 接口与页面脚本数据流。将浅色背景规则替换为与现有 aurora 主题一致的深蓝半透明面板和高对比文本。
...
- Modify: `templates/inbound.html:171-176`
...
将历史项、进度统计、汇总、结果框、条码标签和产品卡同步改为同一深色面板体系；删除 GYJ 状态卡对应 HTML。
