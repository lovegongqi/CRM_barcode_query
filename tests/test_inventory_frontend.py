...
class InventoryFrontendBehaviorTests(unittest.TestCase):
    ...
    def test_product_card_expansion_survives_poll_render(self):
        self.run_node(
            ...
            const items = [
                ...
                {
                    barcode: 'B', name: '乙', state: 'serial_pending', has_serial: true,
                    serial_synced_at: '2026-09-09T10:22:00', serial_expected_count: 1277,
                    book_qty: 2, actual_qty: 1, diff_qty: -1,
                },
            ];
    ...
    def test_count_dialog_is_immediately_editable_without_gyj_request(self):
        self.run_node(
            r"""
            class FakeNode {
                constructor() {
                    this.children = []; this.open = false; this.value = '';
                    this.disabled = false; this.textContent = ''; this.className = '';
                    this.dataset = {}; this.parentElement = {className: '', classList: {add() {}}};
                }
                append(...nodes) { this.children.push(...nodes); }
                replaceChildren(...nodes) { this.children = [...nodes]; }
                showModal() { this.open = true; }
            ...
            vm.runInContext(source, context);
            vm.runInContext(`
                inventoryTask = {task_id: 'T1', items: [{
                    barcode: 'A1', name: '商品', state: 'pending', latest_book_qty: '12',
                    count_total: null, count_entries: [],
                }]};
                renderCountProduct = () => {};
                renderCountEntries = () => {};
                renderInventoryItems = () => {};
            ...
            const opening = vm.runInContext("openCountItem('A1')", context);
            assert.equal(element('inventoryCountDialog').open, true);
            assert.equal(element('inventoryCountNewQuantity').disabled, false);
            assert.equal(element('inventoryCountAdd').disabled, false);
            assert.equal(element('inventoryCountNewQuantity').focused, true);
            assert.match(element('inventoryCountMessage').textContent, /立即录入/);
            assert.equal(requestCount, 0);
            Promise.resolve(opening).catch(error => { console.error(error); process.exitCode = 1; });
            """
        )
    ...
    def test_count_entry_mutation_forces_full_task_summary_refresh(self):
        self.run_node(
            ...
            (async () => {
                await vm.runInContext('addCountEntry()', context);
                for (let i = 0; i < 10 && element('inventoryTaskSummary').children.length < 2; i += 1) {
    ...
    def test_count_entry_refresh_failure_keeps_saved_add_and_blocks_duplicate_request(self):
        self.run_node(
            ...
            (async () => {
                ...
                assert.equal(element('inventoryCountMessage').textContent, '已加入这一笔数量。');
    ...
    def test_update_and_delete_count_entries_force_full_task_refresh(self):
        self.run_node(
            ...
            (async () => {
                await vm.runInContext("updateCountEntry({entry_id: 9, version: 3}, '8')", context);
                for (let i = 0; i < 10 && rendered.length < 1; i += 1) await new Promise(setImmediate);
                await vm.runInContext("deleteCountEntry({entry_id: 9, version: 4})", context);
                for (let i = 0; i < 10 && rendered.length < 2; i += 1) await new Promise(setImmediate);
    ...
    def test_serial_dialog_uses_prefetched_cache_without_opening_refresh(self):
        self.run_node(
            r"""
            ...
            const context = {
                ...
                fetch: async (url, options) => {
                    requests.push({url, options});
                    if (url.endsWith('/serial/open')) return response({
                        success: true, version: 4,
                        serial: {
                            barcode: 'B2', item: {
                                barcode: 'B2', state: 'serial_pending',
                                serial_synced_at: '2026-09-09T10:22:00',
                                serial_expected_count: 1277,
                            },
                            counts: {}, expected: [], matched: [], system_only: [],
            ...
            (async () => {
                ...
                assert.match(element('inventorySerialMessage').textContent, /已载入序列号缓存/);
                assert.match(element('inventorySerialMessage').textContent, /1277/);
            })().catch(error => { console.error(error); process.exitCode = 1; });
            """
        )

    def test_scanner_gun_mode_suppresses_mobile_keyboard_for_serial_fields(self):
        self.run_node(
            r"""
            function field() {
                return {
                    inputMode: 'text', attributes: {},
                    setAttribute(name, value) { this.attributes[name] = String(value); },
                    removeAttribute(name) { delete this.attributes[name]; },
                };
            }
            const button = {
                textContent: '扫码枪模式', attributes: {},
                setAttribute(name, value) { this.attributes[name] = String(value); },
            };
            const elements = new Map([
                ['inventorySerialScannerMode', button],
                ['inventorySerialInput', field()],
                ['inventoryCartonStartSerial', field()],
                ['inventoryCartonExtraSerial', field()],
            ]);
            const context = {
                console, URLSearchParams, encodeURIComponent, BigInt, Uint8Array,
                document: {
            ...
            vm.runInContext('toggleSerialScannerMode()', context);
            assert.equal(button.textContent, '扫码枪已开启');
            assert.equal(button.attributes['aria-pressed'], 'true');
            for (const id of ['inventorySerialInput', 'inventoryCartonStartSerial', 'inventoryCartonExtraSerial']) {
                assert.equal(elements.get(id).inputMode, 'none');
                assert.equal(elements.get(id).attributes.virtualkeyboardpolicy, 'manual');
            }

            vm.runInContext('toggleSerialScannerMode()', context);
            assert.equal(button.textContent, '扫码枪模式');
            assert.equal(button.attributes['aria-pressed'], 'false');
            for (const id of ['inventorySerialInput', 'inventoryCartonStartSerial', 'inventoryCartonExtraSerial']) {
                assert.equal(elements.get(id).inputMode, 'text');
                assert.equal(elements.get(id).attributes.virtualkeyboardpolicy, undefined);
            }
            """
        )
    ...
    def test_serial_refresh_preserves_unsubmitted_scan(self):
        self.run_node(
            ...
            (async () => {
                ...
                assert.equal(vm.runInContext('currentSerialData.counts.matched', context), 4);
            })().catch(error => { console.error(error); process.exitCode = 1; });
            """
        )
    ...
    def test_manual_serial_refresh_is_the_only_forced_refresh(self):
        self.run_node(
            ...
            (async () => {
                ...
                assert.equal(requests.length, 2);
                assert.equal(requests[0].url, '/api/inventory/tasks/T1/items/B2/serial/refresh');
                assert.deepEqual(JSON.parse(requests[0].options.body), {
                    device_id: 'device-a', force: true,
                });
                assert.equal(requests[1].url, '/api/inventory/tasks/T1/items/B2/serial/open');
    ...
    def test_serial_scan_failure_hides_raw_backend_error_and_explains_gyj_retry(self):
        self.run_node(
            r"""
    ...
    def test_refresh_started_before_delete_cannot_replace_delete_result(self):
        self.run_node(
            ...
            (async () => {
                ...
                assert.deepEqual(rendered, ['DELETED', 'DELETED']);
